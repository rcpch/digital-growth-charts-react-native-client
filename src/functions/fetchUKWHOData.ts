...
function fetchUKWHOData(
  ...
  const truncatedPretermDataSet = uk90PretermDataSet.filter((centile: any) => {
    return filterData(centile.data, lowerX, upperX, lowerY, upperY);
  });
  const truncatedUKWHOInfantDataSet = ukwhoInfantDataSet.filter(
    (centile: any) => {
      return filterData(centile.data, lowerX, upperX, lowerY, upperY);
    },
  );
  const truncatedUKWHOChildDataSet = ukwhoChildDataSet.filter(
    (centile: any) => {
      return filterData(centile.data, lowerX, upperX, lowerY, upperY);
    },
  );
  const truncatedUK90ChildDataSet = uk90ChildDataSet.filter((centile: any) => {
    return filterData(centile.data, lowerX, upperX, lowerY, upperY);
  });
