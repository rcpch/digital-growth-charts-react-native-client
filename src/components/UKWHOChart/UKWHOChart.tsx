...
import {setTermDomainsForMeasurementMethod} from '../../functions';
...
// subcomponents
import {AppText, XPoint, ChartCircle, MonthsLabel} from '../SubComponents';
...
import {setTermXDomainsByMeasurementAges} from '../../functions/';
import {xTickCount} from '../../functions';

const VictoryZoomVoronoiContainer = createContainer('zoom', 'voronoi'); // allows two top level containers: zoom and voronoi
...
function UKWHOChart({
  ...
  const getEntireYDomain = setTermDomainsForMeasurementMethod(
    measurementMethod,
    20,
    'uk-who',
  ); // sets max y value for measurement

  return (
    <View style={styles.wholeChart}>
      ...
      <VictoryChart
            ...
            labels={({datum}) => {
              ...
              if (datum.centile_band) {
                ...
                if (datum.age_type === 'corrected_age') {
                  ...
                  let finalString = datum.lay_decimal_age_comment
                    .replace(', ', ',\n')
                    .replace('. ', '.\n');
                  return (
                    'Corrected age: ' +
        ...
        {(domains.x[1] > 2 ||
          (allMeasurementPairs.length > 0
            ? allMeasurementPairs[allMeasurementPairs.length - 1][0]['x'] > 2
            : false)) && ( //render years x axis only if upper domain > 2 or highest supplied measurement > 2y
          <VictoryAxis
            minDomain={0}
            label="Age (years)"
            style={{
            ...
            style={{
              ...
              grid: {
                stroke: () =>
                  gridlineStyle.gridlines ? gridlineStyle.stroke : null,
                strokeWidth: gridlineStyle.strokeWidth,
                strokeDasharray: gridlineStyle.dashed ? '5 5' : '0 0',
              },
            }}
            tickLabelComponent={
              <ChartCircle
          ...
          pubertyThresholdBoys.map((data, index) => {
            return (
              <VictoryAxis
                ...
                label={data.label}
                axisLabelComponent={
                  <VictoryLabel dy={30} dx={-50} textAnchor="start" />
                }
                style={{
                  axis: {
                    stroke: 'black',
        ...
        {centileData.map((reference, index) => {
          if (index === 0 && !isPreterm) {
            // do not want to render preterm data if this is not a preterm child - this will leave a 2 week gap from 0 to 2 weeks
            return;
          }
...
//   />
//   {isPreterm && (
//     <button onClick={onClickShowPretermChartHandler}>
//       {showPretermChart ? (
//         <>View Childhood Chart</>
//       ) : (
//         <>View Preterm Chart</>
//       )}
//     </button>
